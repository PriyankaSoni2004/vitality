...
def dummy(file):
    ...
    sr,X_init=file
    X=(3.0517578e-05)*X_init
    print(sr)
    print(X)
    print(len(X))
    print(len(file[1]))
    print(len(X)==len(file[1]))
    print(type(X))
    print(type(file[1]))
    stft = np.abs(librosa.stft(X))
...
iface=gr.Interface(fn=dummy,inputs= gr.inputs.Audio(label="Upload Audio File"),outputs = gr.outputs.Textbox(label="Processed Text"))
iface.launch()